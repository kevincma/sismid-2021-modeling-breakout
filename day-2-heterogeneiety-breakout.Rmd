...
# Simulate either variable susceptibility or exposure models given 
# susceptibility or activity ratios, respectively
#
# INPUT 
# R0.value = desired R0
...
# plot.days = how many days to plot for if plots are T
...
run.structured.model <- function(R0.value, pop, pop.p, g.parm, fitted.vars, model='exposure',
    ...
    if (model=='suscep'){
        hazards <- fitted.vars
        scale.beta <- hazards*matrix(1, 5, 5)/pop
        scaling.factor <- rescale.R0(scale.beta, g.parm, pop.p, N, R0.value)
        parms <- list(r=r.parm, g=g.parm, hazards=hazards, pop=pop, scaling.factor=scaling.factor)
        simulation <- as.data.frame(lsoda(inits, dt, suscep.SEIR, parms=parms))
    } else if (model=='exposure'){
        activities <- fitted.vars
        scale.beta <- (1-epsilon)*outer(activities, activities)/sum(pop*pop.p*activities) +
                      epsilon*activities/(pop*pop.p)*diag(5)
        scaling.factor <- rescale.R0(scale.beta, g.parm, pop.p, N, R0.value) 
        parms <- list(r=r.parm, g=g.parm, activities=activities, 
    ...
    names(results) <- c('Final epidemic size', 'HIT', 'HIT by group')
    return(results)
}
...
fit.model <- function(pop, pop.p, sero.p, sero.tested, r.parm, g.parm, epsilon=0, years=1, 
                      study.time=100, alpha=1, t.a=0, t.b=0, init.inf=rep(1,5), model='exposure') {
    
    fit.func <- function(par){
        parms <- list(r=r.parm, g=g.parm, params=par, scaling.factor=1, 
                      pop=pop, pop.p=pop.p, epsilon=epsilon, alpha=alpha, t.a=t.a, t.b=t.b)
    ...
    #print(paste(c('AIC:', round(2*length(fit$par)+2*fit$value, 3)), collapse=' '))
    #print(paste(c('Fitted values:', round(fit$par, 3)), collapse=' '))
...
activities.li <- fit.model(pop=pop.li, pop.p=pop.p.li, sero.p=sero.p.li, 
                           sero.tested=sero.tested.li, r.parm=r.parm, g.parm=g.parm)      

# Simulate the model with R0=3 and plot outputs / print results (HIT, final epidemic size)
results.pmix.li <- run.structured.model(R0.value=default.R0, pop=pop.li, 
                                pop.p=pop.p.li, g.parm=g.parm, fitted.vars=activities.li)
...
# Fit the activity levels using maximum likelihood
activities.li.amix <- fit.model(pop=pop.li, pop.p=pop.p.li, sero.p=sero.p.li, epsilon=0.5,
                           sero.tested=sero.tested.li, r.parm=r.parm, g.parm=g.parm)      

# Simulate the model with R0=3 and plot outputs / print results (HIT, final epidemic size)
results.amix.li <- run.structured.model(R0.value=default.R0, pop=pop.li, epsilon=0.5, 
                                pop.p=pop.p.li, g.parm=g.parm, fitted.vars=activities.li.amix)
